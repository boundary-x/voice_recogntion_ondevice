...
let worker, ready=false, busy=false, recording=false, stream, context, source, processor, silenceGain, timer, chunks=[], started=0,lastVoice=0,voiced=false, generation=0,loadStart=0, pending, device, activeModel;
const records=[];
...
function initWorker(){
 ...
 worker.onmessage=({data:d})=>{
  ...
  if(d.type==='result'){
   busy=false;controls();const text=d.text.trim();const command=match(text);$('heard').textContent='들은 말: '+(text||'(없음)');$('command').textContent=`명령 후보: ${command||'일치 없음'} · 전송하지 않음`;
   ...
   const row={...pending,model:activeModel,language:$('language').value,text,command,inferenceMs:Math.round(d.ms),resultLatencyMs:Math.round(performance.now()-pending.end),bluetoothConnected:!!device?.gatt.connected,time:new Date().toISOString()};records.push(row);
   const tr=document.createElement('tr');[row.model,row.kind,text,(d.ms/1000).toFixed(2)+'초',row.bluetoothConnected?'연결됨':'미연결'].forEach(t=>{const td=document.createElement('td');td.textContent=t;tr.append(td);});$('rows').prepend(tr);
  }
 };
}
function match(text){const norm=s=>s.replace(/[\s.,!?。！？]/g,'');const mapping={forward:['전진','앞으로','직진','출발'],backward:['뒤로','후진'],stop:['멈춰','정지','그만'],left:['좌회전','왼쪽','좌측'],right:['우회전','오른쪽','우측']};return Object.entries(mapping).find(([,v])=>v.some(p=>norm(p)===norm(text)))?.[0]||null;}
...
$('ble').onclick=async()=>{try{if(!navigator.bluetooth)throw new Error('이 브라우저에는 Web Bluetooth가 없습니다');if(device?.gatt.connected)device.gatt.disconnect();device=await navigator.bluetooth.requestDevice({filters:[{namePrefix:'BBC micro:bit'}],optionalServices:['6e400001-b5a3-f393-e0a9-e50e24dcca9e']});device.addEventListener('gattserverdisconnected',()=>$('bleStatus').textContent='연결 해제됨');await device.gatt.connect();$('bleStatus').textContent=device.name+' 연결됨 · 데이터 전송 없음';}catch(e){$('bleStatus').textContent=e.message;}};
